...
class PurchaseOrderLineController extends AppController
{
	...
	public function actionDhtmlxgrid($un=NULL, $posStart=0, $count=0,$search=NULL,$mainid=0){
		...
		foreach($provider->getModels() AS $record){
			if(!is_null($record))
			{
				...
				$record['deletelink'] = Html::url(['/purchase/purchase-order-line/window','id'=>$record['id'],'win'=>'purchaseOrderLine_deleteajax','mainid'=>$mainid]);
  ...
  public function actionJsonlist($search = NULL)
  {
    ...
    if(!is_Null($search))
    {
      $mainQuery = $query->select('article AS text')->distinct()
        ->from('tbl_article')
